...
def _ensure_path_record(conn: sqlite3.Connection, path_elements):
    ...
    if row:
        ...
        c.execute(
            "INSERT INTO paths (path, count, lastmod) VALUES (?, ?, ?)",
            (path_str, 1, now_ts),
        )
        pid = c.lastrowid
    conn.commit()
...
def _needs_new_trace(conn: sqlite3.Connection, path_id: int, now_ts: str) -> bool:
    ...
    - Wenn letzter Trace älter als 1 Stunde -> True
    """
    c = conn.cursor()
    c.execute(
    ...
    return delta.total_seconds() > 3600  # > 1 Stunde
...
async def main():
    ...
    parser.add_argument("--db", default="mcrgraph.sqlite", help="SQLite database file")
